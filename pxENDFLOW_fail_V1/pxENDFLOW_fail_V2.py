...
def current_txt_file():
    path = os.path.abspath('.')
    paths = os.listdir(path)
    # print(paths)
    files = []
    for p in paths:
        p_abs = os.path.join(path, p)
        print(p_abs)
        if os.path.isfile(p_abs):
            files.append(p_abs)
    txt_files = list(filter(lambda x: str(x).lower().endswith('.txt'), files))
    return txt_files


def child_txt_file(all_files, first_files):
    txt_files = []
    for f in all_files:
        if f not in first_files:
            txt_files.append(f)
    return txt_files
...
def main():
    ...
    first_txt_files = current_txt_file()
    txt_files = list(filter(lambda x: str(x).lower().endswith('.txt'), all_files))
    txt_files = child_txt_file(txt_files, first_txt_files)
